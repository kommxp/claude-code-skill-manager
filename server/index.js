...
/**
 * Initialize: load cache -> incremental parse history -> scan skills -> merge (初始化：加载缓存 → 增量解析 history → 扫描 skill → 合并)
 */
async function initialize() {
  console.log('[init] Loading data...');
  const startTime = Date.now();

  // 1. Load stats cache (加载 stats 缓存)
  const statsCache = loadStatsCache();

  // 2. Incremental parse history (增量解析 history)
  ...
  // 4. Scan local skills (扫描本地 skill)
  cache.skills = scanAllSkills();

  // 5. Aggregate stats (聚合统计)
  cache.statsMap = aggregateStats(cache.calls);
  mergeStatsToSkills(cache.skills, cache.statsMap);

  // 6. Generate overview (生成总览)
  cache.overview = generateOverview(cache.skills, cache.calls);

  cache.initialized = true;
  const elapsed = Date.now() - startTime;
  console.log(`[init] Loading complete (${elapsed}ms) — ${cache.skills.length} skills, ${cache.calls.length} calls`);
}

/**
 * Refresh data (incremental) (刷新数据（增量）)
 */
...
function resetIdleTimer() {
  ...
  idleTimer = setTimeout(() => {
    ...
    if (idle >= IDLE_TIMEOUT) {
      console.log(`\n[idle] No requests for ${IDLE_TIMEOUT / 60000} min, auto-closing`);
      process.exit(0);
    }
  }, IDLE_TIMEOUT + 1000);
...
// Inject cache and refresh into request context, also track activity (把 cache 和 refresh 注入到请求上下文，同时追踪活跃)
app.use((req, res, next) => {
  req.cache = cache;
  req.refresh = refresh;
  // Only count frontend pages and API requests as activity (exclude favicon and other static assets) (只有前端页面和 API 请求才算活跃（排除 favicon 等静态资源）)
...
// Static file hosting (Vite build output, implemented in M2) (静态文件托管（Vite 构建产物，M2 时实现）)
...
// ============================================================
/**
 * Find an available port (starting from basePort, try up to 10) (找一个可用端口（从 basePort 开始，最多试 10 个）)
 */
function findAvailablePort(basePort) {
  ...
  return new Promise((resolve) => {
    ...
    function tryPort() {
      if (port > basePort + 10) {
        resolve(basePort); // All tried, use default (都试完了，用默认的)
        return;
      }
...
async function start() {
  ...
  // Start online skill index engine (background auto-discover & maintain) (启动在线 skill 索引引擎（后台自动发现 & 维护）)
  const config = loadStartupConfig();
  startIndexEngine(config.githubToken);

  // Start description enrichment engine (background slow-fill translation + use cases) (启动描述富化引擎（后台慢补翻译 + 使用场景）)
  startEnricher(() => getOnlineSkills(config.githubToken));

  // Start tagging engine (background slow-fill AI auto-tagging) (启动标签引擎（后台慢补 AI 自动打标签）)
  startTagger(() => getOnlineSkills(config.githubToken));

  // Auto-find available port (自动找可用端口)
  ...
  app.listen(port, '127.0.0.1', () => {
    ...
    if (port !== BASE_PORT) {
      console.log(`   (Port ${BASE_PORT} occupied, using ${port})`);
    }
    console.log(`   Auto-close after ${IDLE_TIMEOUT / 60000} min of inactivity\n`);

    // Auto-open browser (unless NO_BROWSER env is set) (自动打开浏览器（除非设置了 NO_BROWSER 环境变量）)
    if (!process.env.NO_BROWSER) {
      openBrowser(url);
    }

    // Start idle timer (启动空闲计时)
    resetIdleTimer();
  });
}
